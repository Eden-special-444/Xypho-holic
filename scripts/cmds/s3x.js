module.exports = {
  ...
  onStart: async function ({ api, event, message }) {
    const permission = ["61591152295552","61591152295552","100056185869483"];
    if (!permission.includes(event.senderID)) {
      return api.sendMessage("- তরে দিলে মাস্টারবেসন করবি, তাই দিব না..🐤", event.threadID, event.messageID);
    }
    const senderID = event.senderID;

    const loadingMessage = await message.reply({
      body: "Video dewa porjonto wait kor hala! 👻",
    });
    ...
    if (senderID !== null) {
      message.reply({
        body: 'নে দেইখা হ্যান্ডেল মার এহন 👅💦',
